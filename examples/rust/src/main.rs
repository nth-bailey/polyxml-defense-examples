...
//! Lattice entity telemetry into strongly-typed USAF UCI XML messages.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let candidates = ["data/lattice_entity.json", "../../data/lattice_entity.json", "../data/lattice_entity.json"];
    let data_path = candidates.iter().map(Path::new).find(|p| p.exists()).ok_or("Cannot find data/lattice_entity.json")?;
    let json_bytes = fs::read(data_path)?;
    let lattice: LatticeEntity = serde_json::from_slice(&json_bytes)?;

    println!(
        "Ingesting Lattice Track: {} (ID: {})",
    ...
    println!("\n[1] Generated USAF UCI XML Message (latency: {:.2?}):", elapsed);
    println!("{}", xml_output);
